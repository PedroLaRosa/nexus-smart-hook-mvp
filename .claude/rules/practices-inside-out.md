...
Follow the TDD cycle defined in @.claude/rules/practices-tdd.md for each layer applying @.claude/rules/practices-testing.md
...
```
Domain (Pure logic) → UseCase → Repository Adapter → UI/Components
```
...
| Hooks | Unit | InMemoryRepositories |
| Repository Adapters | Integration | Real API |
| UI/Components | E2E | Playwright |
...
Unit tests using **InMemoryRepositories** and `@testing-library/react`.

```typescript
// Hook test - uses InMemoryRepository
...
Test against **real API** (dev/sandbox environment).

```typescript
// Repository adapter test - real API
...
Test the **full UI** through the browser with Playwright.

```typescript
// E2E test - full UI flow
import { test, expect } from '@playwright/test';

test.describe('Order Creation', () => {
  test('creates an order via UI', async ({ page }) => {
...
Follow the 5-step TDD cycle defined in @.claude/rules/practices-tdd.md for each layer:
...
7. Implement Repository Adapters
   -> Integration tests with real API

8. Wire up UI/Components
...
3. Test repository adapters against real APIs
